...
{
    internal class HttpWebSocketSupports : IHttpWebSocketSupports
    {
        ...
        /// <summary>
        /// Initialize a new <see cref="HttpWebSocketSupports"/> instance.
        /// </summary>
        /// <param name="Http"></param>
        public HttpWebSocketSupports(IHttpContext Http)
        {
            m_Http = Http;
        }
        ...
        public bool IsWebSocketRequest
        {
            get
            {
                ...
                var Upgrade = m_Http.Request.Headers.Get("Upgrade");
        ...
        public IEnumerable<string> Subprotocols
        {
            get
            {
                ...
                {
                    var Subprotocol = m_Http.Request.Headers.Get("Sec-WebSocket-Protocol", null);
        ...
        /// <inheritdoc/>
        public Task<WebSocket> AcceptAsync(string Subprotocol = null)
        {
            if (!IsWebSocketRequest)
                throw new InvalidOperationException("The request isn't a WebSocket request.");

            var Opaque = m_Http.GetFeature<IHttpOpaqueFeature>();
            try
            {
                while(true)
                {
                    ...
                    var Stream = Opaque.GetOpaqueStream();
                    var Socket = WebSocket.CreateFromStream(Stream,
            ...
            try   { Opaque.GetOpaqueStream().Close(); }
            catch { }
        ...
        private bool CheckSupports()
        {
            ...
            Span<byte> Temp = stackalloc byte[16]; m_RequestKey = Headers.Get("Sec-WebSocket-Key");
        ...
        {
            var Headers = m_Http.Request.Headers;
            var Values = (Headers.Get(Header) ?? "").Trim().Split(',')
                .Where(X => !string.IsNullOrWhiteSpace(X)).ToArray();

            if (Values is null || Values.Length <= 0)
                return false;

            if (Values.Count(X => X.Equals(Header, StringComparison.OrdinalIgnoreCase)) > 0)
            {
                if (Values.Length == 1)
